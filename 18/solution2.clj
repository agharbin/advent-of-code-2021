...
(defn compute-explode [t]
  (let [marked-tree (mark-explodes t)
    ...
    (parse-tree (concat (add-to-back prefix l-value) [0] (add-to-front postfix r-value)))))
...
(defn compute-split [t]
  (let [marked-tree (mark-splits t)
        ...
        postfix (->> marked-tree (drop-while #(false? (second %))) (drop 1) (map first) flatten-tree)]
    (parse-tree (concat prefix (get-split-pair val-to-split) postfix))))
...
(defn solve [input]
  (->> (for [x input y input :when (not= x y)] [y x])
       (map #(apply add-lists %))
       (map compute-magnitude)
       (apply max)))
